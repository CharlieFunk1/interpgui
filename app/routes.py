...
@app.route('/setup_snake/<strip_number>', methods=['GET', 'POST'])
def setup_snake(strip_number):
    form_general = StripFormSnakeGeneral()
    form_segment = StripFormSnakeSegement()
    configure = Configure.query.get(1)
    strip = StripSnake.query.filter_by(strip_num=strip_number).first()
    strips = StripSnake.query.all()
    opencv_draw(strips)

    if form.validate_on_submit():
        try:
            strip = StripSnake.query.filter_by(strip_num=form.strip_num.data).first()
            db.session.delete(strip)
            db.session.commit()
        except:
            print("No data to delete")
            
        strip = Strip(strip_num = form_general.strip_num.data,
            strip_segments = form_general.strip_segments.data,
            start_pos_x = form_general.start_pos_x.data,
            start_pos_y = form_general.start_pos_y.data,
            line_color_r = form_general.line_color_r.data,
            line_color_g = form_general.line_color_g.data,
            line_color_b = form_general.line_color_b.data,
            ip = form_general.ip.data)

        db.session.add(strip)
        db.session.commit()
        flash('Config Data Submitted for Strip {}'.format(
            form_general.strip_num.data))
        return redirect('/setup_snake_segments/' + str(form_general.strip_num.data))
    try:
        if str(strip.strip_num).isdigit() == True:
        
            form_general.strip_num.data = strip.strip_num
            form_general.strip_segements.data = strip.strip_segemnts
            form_general.start_pos_x.data = strip.start_pos_x
            form_general.start_pos_y.data = strip.start_pos_y
            form_general.line_color_r.data = strip.line_color_r
            form_general.line_color_g.data = strip.line_color_g
            form_general.line_color_b.data = strip.line_color_b
            form_general.ip.data = strip.ip
    except:
        print("No data present in strip")

    return render_template('setupsnake.html', title='Interp Setup', form=form, configure=configure)


@app.route('/setup_snake_segments/<strip_number>', methods=['GET', 'POST'])
def setup_snake_segments(strip_number):
    form_general = StripFormSnakeGeneral()
    form_segment = StripFormSnakeSegement()
    configure = Configure.query.get(1)
    strip = StripSnake.query.filter_by(strip_num=strip_number).first()
    ...
    return render_template('setupsnakesegments.html', title='Interp Setup', form=form, configure=configure)
